...
import { useRealtimeHarvestNotifications } from '@/hooks/useRealtimeHarvestNotifications';
...
export function NotificationBell({ className }: { className?: string }) {
  ...
  // Global unread count + realtime updates
  const { unreadCount, refreshUnreadCount } = useRealtimeHarvestNotifications();

  const loadNotifications = useMemo(() => {
    return async () => {
      setLoading(true);
      try {
        const notificationsData = await getMyHarvestNotifications();
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-start gap-3 flex-1">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className={cn('text-sm mb-2', !notification.isRead && 'font-medium')}>
                            ...
                            <div className="flex items-center gap-4 text-xs text-gray-500">
                              ...
                              <span>{notification.cropBatch.farm.name}</span>
